...
passwd_entry_t* parse_passwd(struct options* options, int* nr_entries)
{
	...
	if ((passwd=fopen(options->infilename,"r"))==NULL) {
		fprintf(stderr, "%s could not be opened: ",options->infilename);
		perror(NULL);
		return NULL;
	}
	...
	memset(entries,0,sizeof(passwd_entry_t)*line_count); // ←←←←←← Pero esto no habíamos dicho que no servía para nada?
		...
		while((token = strsep(&lineptr, ":"))!=NULL) {
			...
			case UID_IDX:
				if (sscanf(token,"%d",&cur_entry->uid)!=1) {
					fprintf(stderr, "Couldn't parse UID field in line %d of %s. token is %s\n",entry_idx+1, options->infilename, token);
					return NULL;
				}
				break;
			case GID_IDX:
				if (sscanf(token,"%d",&cur_entry->gid)!=1) {
					fprintf(stderr, "Couldn't parse GID field in line %d of %s. token is %s\n",entry_idx+1, options->infilename, token);
					return NULL;
				}
				break;
			case USER_NAME_IDX:
